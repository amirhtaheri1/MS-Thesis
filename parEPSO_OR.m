function out = parEPSO_OR(problem, parameters, data, size_parameters)
% out = parEPSO_OR(problem, parameters, data, size_parameters)
% minimizing cost function for optimal rule_set in FLC
...
nVar = problem.rule_nVar;    % search space dimension of FLC rules;         % search space dimension / or DM
Var_size = [1 nVar];         % matrix size of decision variables

% lower bound of decision variables
Var_min = problem.rules_min;
% upper bound of decision variables
Var_max = problem.rules_max;
...
parfor k=1:nPop
    for j=1:nVar
        particle(k).position(j) = randi(Var_max(j),1);
    end
    ...
    % evaluation
    x = [size_parameters, particle(k).position];
...
% using the un-optimized rule_set as initial data (particle(1))
if parameters.use_initial_guess
    particle(1).position = rule_set_initial;
    
    % evaluation
    x = [size_parameters, rule_set_initial];
    particle(1).cost = CostFunction(x,data);
        
    % initial update personal best
    particle(1).best.position = rule_set_initial;
...
M.position = round(M.position);

x = [size_parameters, M.position];
...
%     tic
    parfor z=1:nPop
        ...
        % sorting particle(i).position for FLC rule-set
        particle(z).position = round(particle(z).position);
        
        % evaluation
        x = [size_parameters, particle(z).position];
...
%     toc
    ...
    for counter=1:nPop
        if (PB(counter)<global_best.cost) && particle(GB_index(counter)).flag
            ...
            % OR calculation happens at this part of parEPSO_OP.m
            break
...
if data.excel==1
    ...
    blanked = full - length(size_parameters) - length(out.best_costs) -length(global_best.position) -7;
    new_file = [last_file;
                11*ones(1,full);
                datetime_value,...
                out.stuck,data.w,parameters.max_iteration,...
                parameters.nPop,parameters.stuck_step,parameters.try_it,...
                out.best_costs',zeros(1,blanked),...
                size_parameters,out.optimal_rule_set;
                11*ones(1,full)];
    xlswrite(filename,new_file,sheet,xlRange);
end

disp('parEPSO_OR ended.')
end

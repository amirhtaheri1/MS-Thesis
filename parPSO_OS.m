function out = parPSO_OS(problem, parameters, data, size_init)
% out = parPSO_OS(problem, parameters, data, size_init)
...
% updated in ver. 19 based on parPSO_OP in ver. 18.

disp('parPSO_OS running...')
...
parfor i=1:nPop        
    particle(i).position = Var_min + rand()*(Var_max - Var_min);
    particle(i).position = FixSize(particle(i).position);
    
    % initial velocity
    particle(i).velocity = zeros(Var_size);
    
    % evaluation
    x = [particle(i).position, parameters_initial, rule_set_initial];
    ...
    for counter=1:nPop
        if (PB(counter)<global_best.cost) && particle(GB_index(counter)).flag
            ...
            while (OFLC.stuck==1) && (try_it<parameters.try_it)
                disp('GB improved');
                OFLC = parPSO_OP(problem, parameters, data, global_best.position);

function out = parPSO_OR(problem, parameters, data, size_parameters)
% out = parPSO_OR(problem, parameters, data, size_parameters)
...
parfor i=1:nPop
    for k=1:nVar
        particle(i).position(k) = randi(Var_max(k),1);
    end
    ...
    % evaluation
    x = [size_parameters, particle(i).position];
...
%     tic
    parfor i=1:nPop
        ...
        % sorting particle(i).position for FLC rule_set
    ...
    for counter=1:nPop
        if (PB(counter)<global_best.cost) && particle(GB_index(counter)).flag
            global_best = tmp(GB_index(counter));
                % OR calculation happens at this part of parPSO_OP.m
            break
...
out.optimal_rule_set = global_best.position;

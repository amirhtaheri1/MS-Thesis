function out = parEPSO_OS(problem, parameters, data, size_init)
% out = parEPSO_OS(problem, parameters, data, size_init)
% minimizing cost function for optimal sizing
...
% size_init is the initial size of the un-optimized HGPS.
% updated in ver. 19 based on parEPSO_OP in ver. 18.

disp('parEPSO_OS running...')

%% Initiallization
rule_set_initial = data.rule_set_initial;
parameters_initial = data.parameters_initial;
OFLC.optimal_parameters = data.parameters_initial;
OFLC.optimal_rule_set = data.rule_set_initial;

%% Definition
load = problem.load;         % load vevtor in kW
nVar = problem.x_nVar;       % search space dimension / or DM
Var_size = [1 nVar];         % matrix size of decision variables

% lower bound of decision variables
Var_min = problem.x_min;
% upper bound of decision variables
Var_max = problem.x_max;
...
parfor k=1:nPop
    particle(k).position = Var_min + rand()*(Var_max - Var_min);
    particle(k).position = FixSize(particle(k).position);
    
    % initial velocity
    particle(k).velocity = zeros(Var_size);
    
    % evaluation
    x = [particle(k).position, parameters_initial, rule_set_initial];
...
if parameters.use_initial_guess
    particle(1).position = size_init;
    
    % evaluation
    x = [particle(1).position, parameters_initial, rule_set_initial];
    particle(1).cost = CostFunction(x,data);
        
    % initial update personal best
    particle(1).best.position = size_init;
...
M.position = FixSize(M.position);

x = [M.position, parameters_initial, rule_set_initial];
...
%     tic
    parfor z=1:nPop
        ...
        particle(z).position = FixSize(particle(z).position);
        
        % evaluation
        x = [particle(z).position, parameters_initial, rule_set_initial];
...
%     toc
    ...
    for counter=1:nPop
        if (PB(counter)<global_best.cost) && particle(GB_index(counter)).flag
            ...
            OFLC.stuck = 0;
            warning('FLC is rigged in parPSO_OS.m // line 204.');
            try_it = 1;
            % optimal rule_set
            while (OFLC.stuck==1) && (try_it<parameters.try_it)
                disp('GB improved');
                OFLC = parEPSO_OP(problem, parameters, data, global_best.position);
                
                % updating global_best              
                if OFLC.best_solution.cost<global_best.cost
                    global_best.cost = OFLC.best_solution.cost;
                    % global_best position(FLC stays the same, however
                    % the equipment sizing gets updated).
...
out.optimal_size = global_best.position;
out.optimal_parameters = OFLC.optimal_parameters;
out.optimal_rule_set = OFLC.optimal_rule_set;
...
if data.excel==1
    ...
    blanked = full - length(out.optimal_size) - length(out.best_costs) -length(OFLC.optimal_parameters) -length(OFLC.optimal_rule_set) -7;
    new_file = [last_file;
                1111*ones(1,full);
                datetime_value,...
                out.stuck,data.w,parameters.max_iteration,...
                parameters.nPop,parameters.stuck_step,parameters.try_it,...
                out.best_costs',zeros(1,blanked),out.optimal_size,OFLC.optimal_parameters,...
                OFLC.optimal_rule_set;
                1111*ones(1,full)];
    xlswrite(filename,new_file,sheet,xlRange);
end

disp('parEPSO_OS ended.')
end
